 ...
 * **Tab-Blöcke:** Gitarren-Tabs (Riffs) stehen zwischen `{start_of_tab}` und `{end_of_tab}` (Kurzform `{sot}`/`{eot}`, optional mit Bezeichnung `{sot: Riff}`) - ChordPro-Standard. Der Inhalt bleibt Zeichen für Zeichen erhalten: keine Akkorderkennung, keine Transposition, kein Timecode, dargestellt als nicht umbrechender, seitlich scrollbarer Monospace-Block. Wie Kommentare lassen sie sich auf Bandmitglieder beschränken: `{sot4marco}` bzw. `{sot4marco,jamie}` (StageBoard-eigene `4`-Konvention wie bei `{cc4...}`), einstellbar per "Sichtbar für" im Kommentare-Tab. Fehlt `{eot}`, endet der Block am nächsten Part. Der Ultimate-Guitar-Import legt Tab-Blöcke automatisch an (für alle).
