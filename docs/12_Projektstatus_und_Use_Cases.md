...
#### 1. Prompter (`prompter`)
- **Was:** Zeigt Titel, Interpret und den ChordPro-Text (Akkorde + Lyrics) des aktuellen Songs bzw. der aktuellen Variante. Zwei Ansichten: **Smooth Scroll** (die aktive Zeile wird pro Tick sanft zur Mitte gezogen, Easing-Faktor 0,08) und **Paginated View** (Seitenwechsel je Abschnitt, Kopfzeile mit Abschnittsname, `n/m` und „next: …"). **Instrumentale Akkordzeilen** ohne Liedtext (Taktnotation `| C   | F   |`, `Em C G  x4`) stehen inline in einer Zeile - Akkorde in Liedtextgröße zwischen den Taktstrichen, wie im Original gesetzt - statt über einer leeren Zeile zu schweben; ein einziger Buchstabe oder eine Ziffer macht eine Zeile wieder zur Liedtextzeile. **Tab-Blöcke** (#278) erscheinen als eigener, nicht umbrechender Monospace-Block (bei Überbreite seitlich scrollbar), ohne Transposition und ohne Timecode; ein an bestimmte Mitglieder gerichteter Block (`{sot4marco}`) wird wie ein adressierter Kommentar nur diesen gezeigt. Key/Tuning/Capo der Variante erscheinen als erste, mitscrollende Zeile - darunter (neu, #59) die **Transpose**- und **Capo**-Stepper samt „Klingende Tonart"-Anzeige. Kommentar-Zeilen (`{comment:}`/`{c:}`, gezielt `{cc4<Name>:}`) werden je aktivem Profil gefiltert (nicht adressierte Kommentare sind gar nicht Teil des Songs; unbekannte Ziel-Namen bleiben sichtbar).
...
- **Wie (UG):** Headless Chrome via `puppeteer-core` (Chrome-Pfad per `CHROME_EXECUTABLE_PATH` oder Standardpfade), liest `window.UGAPP.store.page.data`, filtert „Pro/Official/Video“, wandelt UG-Markup (`[ch]G[/ch]` über der Textzeile) per `convertUltimateGuitarContent` in ChordPro (`[G]text`, `{part: Verse 1}`). Seit #277 auch Akkordzeilen mit Taktstrichen, Klammern, Wiederholungszeichen, Anmerkungen, `N.C.` und nicht markierten Akkordnamen; `[Chorus] (x4)` wird Part + Kommentar; eckige Klammern im Liedtext werden zu runden. Riffs (≥2 Saitenzeilen, mit Akkordzeile darüber und Zählzeile darunter) werden Tab-Blöcke. Akkorde bleiben exakt in ihrer Originalspalte, auch mitten im Wort (bewusst). Reifeprüfung 2026-09-26 an 35 echten Tabs (Band-Repertoire + 6 bekannte): 0 verlorene Akkorde von 3.486, kein UG-Markup im Ergebnis (vorher 23 von 35 betroffen, ~15 % Akkorde verloren). Liefert zusätzlich Key, Tuning, Capo, BPM. **Wie (MB):** REST gegen `musicbrainz.org/ws/2`, `User-Agent` gesetzt, 10 Treffer.
- **Grenzen:** UG-Scraper hängt an UG-Seitenstruktur und einer lokal installierten Chrome (`--no-sandbox`); bei Änderungen bricht er. Robustheit (#15) offen. „Official“- und „Pro“-Versionen (UGs lizenzierte Transkriptionen) liefern keinen Akkordtext: deren Akkordansicht entsteht nur im Browser eines eingeloggten Pro-Kontos und wird bewusst nicht ausgelesen - importiert wird dann die öffentliche „Chords“-Version desselben Songs. Info-Zeilen am Liedanfang (Interpret, Album, Links, Griffbilder) bleiben im Text. Kein Rate-Limit/Caching. MusicBrainz: keine Retries.
