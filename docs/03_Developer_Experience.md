...
1. **Port 443 statt 3001** (damit der Name allein reicht, ganz ohne `:3001` in der Adresse - Browser probieren bei `https://` ohnehin automatisch Port 443): Ports unter 1024 sind privilegiert, ein normaler `node`-Prozess darf sie ohne Weiteres nicht binden. Einmalig, pro `node`-Binary (bei jedem `nvm install` einer neuen Version erneut nötig):
   ...
   ```
   Danach `core-backend` mit `PORT=443`, `LAN_IP=<Stage-Server-IP>` und `STAGEBOARD_WORKSPACE=<echte Band-Workspace-ID>` starten - `packages/stage-pwa/.env`s `VITE_STAGE_SERVER_URL` entsprechend auf `https://stageboard.local` (ohne Port) setzen und `stage-pwa` neu bauen, da Vite Umgebungsvariablen zur Build-Zeit fest einbackt, nicht zur Laufzeit liest. `STAGEBOARD_WORKSPACE` ist kein optionaler Tuning-Parameter: ohne sie läuft weder Plugin-Sync noch Discovery Modes Server-Teilnehmer (`main()` in `index.ts` prüft `process.env.STAGEBOARD_WORKSPACE` und überspringt beides sonst komplett) - der Server startet, sieht nach außen gesund aus, aber `GET /plugins` bleibt für immer `[]` und jeder `/plugins/:name/trigger`-Aufruf schlägt mit "Unknown plugin" fehl, egal was die Band in der PWA installiert hat. Live gefunden, 2026-09-10: ein Server-Neustart ohne diese Variable (der bisherige Restart-Hinweis hier erwähnte sie schlicht nicht) lief tagelang unauffällig, bis der erste tatsächliche Plugin-Trigger fehlschlug. Die Workspace-ID selbst lässt sich über `GET /workspaces` aus dem passenden Bandnamen ablesen, oder über das jeweilige `workspace:access`-Dokument (`GET /<couch-url>/stageboard-<id>/workspace:access`) aus dem bekannten Zugangscode.
