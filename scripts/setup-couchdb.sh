...
curl -s -X PUT -u "$AUTH" "${COUCHDB_URL}/_node/_local/_config/httpd/enable_cors" -d '"true"' > /dev/null
curl -s -X PUT -u "$AUTH" "${COUCHDB_URL}/_node/_local/_config/cors/origins" -d "\"${FRONTEND_ORIGIN}\"" > /dev/null
curl -s -X PUT -u "$AUTH" "${COUCHDB_URL}/_node/_local/_config/cors/credentials" -d '"true"' > /dev/null
curl -s -X PUT -u "$AUTH" "${COUCHDB_URL}/_node/_local/_config/cors/methods" -d '"GET, PUT, POST, HEAD, DELETE"' > /dev/null
curl -s -X PUT -u "$AUTH" "${COUCHDB_URL}/_node/_local/_config/cors/headers" \
